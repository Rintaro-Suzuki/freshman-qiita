...
## ブラックジャックのルール
今回は難しいルールを考えることなくプレーヤーとディーラーの共通の要素として、21に近い方が勝ち、22以上になったら負けというシンプルなルールのみを考えます。

1. 実行開始時、プレーヤーとディーラー(CPU)はそれぞれカードを2枚引く。引いたカードは画面に表示する。ディーラーの2枚目のカードはわからないようにする。

## CPUの戦略
CPUの戦略について考えます。
CPUの戦略は次のようにしましょう。
「CPUは引いたカードの和が17を超えていたら引くのをやめる。」
